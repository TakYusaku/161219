...
function showText() {
  ...
  for (var i = 0, len = localStorage.length; i < len; i++) {
      html.push('<p>' + '<p style="border-left: 15px solid #9caeb7;padding: 7px;background: #d0dee5;">' + "&nbsp;&nbsp;" + "期限 :" + '<u>' + Math.floor(parseInt(box[i].date, 10) / 10000) + "-" + ('0' + Math.floor((parseInt(box[i].date, 10) % 10000) / 100)).slice(-2) + "-" + ('0' + (parseInt(box[i].date, 10) % 10000) % 100).slice(-2) + '</u>' + '</p>' + " " + '<h2 style="border-bottom: 1px solid #c85179;border-left: 10px solid #c85179;padding: 7px;">' + '<input id="cb" type="checkbox"> '  + "&nbsp" + box[i].value +'</h2>' + '</p>'+'<br>');
  }
